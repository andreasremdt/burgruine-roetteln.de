  ...
  return (
    <section className="py-24 lg:py-40" id="arbeitsgruppe">
      <Container>
        ...
        <p className="mb-16 max-w-1/2 text-5xl text-neutral-900">{description}</p>

        <div className="ml-[35%]">
          <dl className="mb-16 grid grid-cols-3 gap-16">
            {statistics?.map((statistic) => (
              <div key={statistic.id} className="flex flex-col-reverse gap-4">
                <dt className="font-sans font-medium text-neutral-600 uppercase">
                  {statistic.label}
                </dt>
                <dd className="text-7xl text-neutral-900">{statistic.value}</dd>
              </div>
            ))}
          </dl>
